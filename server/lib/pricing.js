...
async function getCoupon(code) {
  if (!code) return null;
  return (await db.prepare('SELECT * FROM coupons WHERE code = ? AND active = 1').get(String(code).toUpperCase().trim())) || null;
}

// Calcula todos os valores. paymentMethod: 'card' | 'pix' | 'mbv'. cep: frete por região. cpf: valida cupom restrito.
async function computeTotals(items, couponCode, paymentMethod, cep, cpf) {
