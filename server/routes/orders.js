...
async function orderWithItems(id) {
  const order = await db.prepare('SELECT * FROM orders WHERE id = ?').get(id);
  if (!order) return null;
  order.items = await db.prepare('SELECT * FROM order_items WHERE order_id = ?').all(id);
  return order;
}
...
async function payCashback(order) {
  if (order.cashback_mbv > 0) {
    await wallet.move(order.user_id, order.cashback_mbv, 'cashback',
...
router.post('/', requireAuth, async (req, res) => {
  ...
  // Estoque (checagem de UX; a garantia real é o decremento condicional na transação)
  ...
  const totals = await computeTotals(items, req.body.coupon_code, method, ship.cep, cpf);
  ...
  if (method === 'mbv' && !onchain) {
    const u = await db.prepare('SELECT mbv_balance FROM users WHERE id = ?').get(req.user.id);
  ...
  const run = db.transaction(async () => {
    ...
    await db.prepare('UPDATE orders SET code = ?, pix_code = ?, chain_id = ?, cpf_cnpj = ? WHERE id = ?')
    ...
    for (const it of items) {
      await insItem.run(orderId, it.product_id, it.name, it.price, it.quantity, it.unit, it.image);
      const dec = await decStock.run(it.quantity, it.product_id, it.quantity);
      if (dec.changes !== 1) {
    ...
    if (method === 'mbv' && !onchain) {
      await wallet.move(req.user.id, -totals.mbvAmount, 'purchase', `Pagamento do pedido ${code}`, code);
    }
    ...
    if (paymentStatus === 'paid') await payCashback(orderRow);
  ...
  try {
    ...
    if (mpPay) {
      const ord = await orderWithItems(orderId);
      const pref = await mp.createPreference(ord, req.user, method);
      return res.status(201).json({ order: ord, redirect: pref.redirect });
    }
    if (paymentStatus === 'paid') email.sendOrderConfirmation(req.user, await orderWithItems(orderId)).catch(() => {});
    res.status(201).json({ order: await orderWithItems(orderId), token: TOKEN, onchain });
  } catch (e) {
...
router.post('/:id/confirm-pix', requireAuth, async (req, res) => {
  ...
  if (order.payment_status === 'paid') return res.json({ order: await orderWithItems(order.id) });
  await db.prepare("UPDATE orders SET payment_status = 'paid' WHERE id = ?").run(order.id);
  await payCashback(order);
  email.sendOrderConfirmation(req.user, await orderWithItems(order.id)).catch(() => {});
  res.json({ order: await orderWithItems(order.id) });
});
...
router.post('/:id/verify-onchain', requireAuth, async (req, res) => {
  ...
  const reused = await db.prepare('SELECT id FROM orders WHERE tx_hash = ? AND id != ?').get(txHash, order.id);
  ...
  try {
    ...
    const upd = await db.prepare("UPDATE orders SET payment_status = 'paid', tx_hash = ?, chain_id = ? WHERE id = ? AND payment_status != 'paid'")
      .run(txHash, CHAIN.chainId, order.id);
    if (upd.changes !== 1) return res.json({ order: await orderWithItems(order.id) }); // já confirmado em paralelo
    email.sendOrderConfirmation(req.user, await orderWithItems(order.id)).catch(() => {});
    res.json({ order: await orderWithItems(order.id), verified: { amount: result.amount, from: result.from } });
  } catch (e) {
    console.error(e);
...
router.get('/', requireAuth, async (req, res) => {
  ...
  if (req.query.all === '1' && req.user.role === 'admin') {
    rows = await db.prepare(`SELECT o.*, u.name AS customer_name, u.email AS customer_email FROM orders o JOIN users u ON u.id=o.user_id ORDER BY o.id DESC`).all();
  } else {
    rows = await db.prepare('SELECT * FROM orders WHERE user_id = ? ORDER BY id DESC').all(req.user.id);
  }
  // Itens de todos os pedidos numa única query (evita N+1 — importante no Postgres)
  const ids = rows.map(o => o.id);
  if (ids.length) {
    const ph = ids.map(() => '?').join(',');
    const items = await db.prepare(`SELECT * FROM order_items WHERE order_id IN (${ph})`).all(...ids);
    const byOrder = {};
    for (const it of items) (byOrder[it.order_id] = byOrder[it.order_id] || []).push(it);
    for (const o of rows) o.items = byOrder[o.id] || [];
  }
...
router.patch('/:id/status', requireAdmin, async (req, res) => {
  ...
  if (req.body.status === 'cancelled' && order.status !== 'cancelled') {
    ...
    const tx = db.transaction(async () => {
      for (const it of items) if (it.product_id) await db.prepare('UPDATE products SET stock = stock + ? WHERE id = ?').run(it.quantity, it.product_id);
      if (order.payment_method === 'mbv' && order.payment_status === 'paid' && order.mbv_amount > 0) {
        await wallet.move(order.user_id, order.mbv_amount, 'refund', `Estorno do pedido ${order.code}`, order.code);
      }
      await db.prepare("UPDATE orders SET status = 'cancelled' WHERE id = ?").run(order.id);
    });
    await tx();
  } else {
    await db.prepare('UPDATE orders SET status = ? WHERE id = ?').run(req.body.status, order.id);
    ...
    if ((req.body.status === 'shipped' || req.body.status === 'delivered') && order.status !== req.body.status) {
      const u = await db.prepare('SELECT * FROM users WHERE id = ?').get(order.user_id);
      const full = await orderWithItems(order.id);
