...
router.get('/stats', async (_req, res) => {
  ...
  const revenue = (await db.prepare(`SELECT COALESCE(SUM(total),0) v FROM orders WHERE ${paidWhere}`).get()).v;
  const ordersCount = (await db.prepare('SELECT COUNT(*) n FROM orders').get()).n;
  const pendingPix = (await db.prepare("SELECT COUNT(*) n FROM orders WHERE payment_status = 'pending'").get()).n;
  const productsCount = (await db.prepare('SELECT COUNT(*) n FROM products WHERE active = 1').get()).n;
  const lowStock = await db.prepare('SELECT id, name, stock, unit FROM products WHERE active = 1 AND stock <= 5 ORDER BY stock ASC').all();
  const customers = (await db.prepare("SELECT COUNT(*) n FROM users WHERE role = 'customer'").get()).n;
  const co2 = (await db.prepare(`
  ...
  // Corte de 7 dias calculado em JS (portável entre SQLite e Postgres)
  const since = db.utcNow(-7 * 24 * 3600 * 1000);
  const salesByDay = await db.prepare(`
    SELECT substr(created_at,1,10) day, COALESCE(SUM(total),0) total, COUNT(*) orders
    FROM orders WHERE ${paidWhere} AND created_at >= ?
