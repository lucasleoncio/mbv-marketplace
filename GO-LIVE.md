...
- **ESCOLHIDO (08/07/2026) — Render Postgres (plano pago já ativo):**
  1. No painel do Render: New → **PostgreSQL** (mesma região do web service — use a *Internal Database URL* para latência mínima).
  2. No web service, defina `DATABASE_URL` com a Internal Database URL (se usar a External, o app ativa TLS sozinho).
  3. O app usa Postgres automaticamente quando `DATABASE_URL` existir; sem ela, segue no SQLite (demo/local). Rollback = remover a env.
  4. Uploads de imagens continuam em disco — adicione um Render Disk (`MBV_UPLOAD_DIR=/var/data/uploads`) ou migre para S3/R2 (item ARQ-9 do plano).
...
4. [ ] Persistência: Render Postgres + `DATABASE_URL` (Internal URL, mesma região) + disco p/ uploads
