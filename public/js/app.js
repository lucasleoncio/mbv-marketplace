...
/* ======================= ADMIN (carregado sob demanda) ======================= */
// O painel admin (~300 linhas) mora em /js/admin.js e só é baixado quando um
// administrador acessa /admin — o cliente comum nunca carrega esse código.
let __adminLoading = null;
function loadAdminBundle() {
  if (window.__mbvAdminReady) return Promise.resolve();
  if (__adminLoading) return __adminLoading;
  __adminLoading = new Promise((resolve, reject) => {
    const s = document.createElement('script');
    s.src = '/js/admin.js'; s.async = true;
    s.onload = resolve; s.onerror = () => reject(new Error('Falha ao carregar o painel.'));
    document.head.appendChild(s);
  });
  return __adminLoading;
}
Pages.admin = async function (parts, query) {
  ...
  loading();
  try { await loadAdminBundle(); } catch (e) { return Pages.error(e); }
  return AdminRouter(parts, query); // definido em /js/admin.js
};
...
/* ---------------- PWA: service worker + convite de instalação ---------------- */
function registerPWA() {
  if ('serviceWorker' in navigator) {
    // registra depois do load para não competir com o primeiro render
    window.addEventListener('load', () => navigator.serviceWorker.register('/sw.js').catch(() => {}));
  }
  // Convite discreto para instalar (Android/desktop). O toast só aparece uma vez.
  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    if (localStorage.getItem('mbv_pwa_dismissed')) return;
    const el = document.createElement('div');
    el.className = 'cookie-bar';
    el.innerHTML = `<span>📲 Instale o app do MBV para comprar mais rápido, direto da tela inicial.</span><span style="display:inline-flex;gap:8px"><button class="btn btn-ghost btn-sm" id="pwaNo">Agora não</button><button class="btn btn-primary btn-sm" id="pwaYes">Instalar</button></span>`;
    document.body.appendChild(el);
    el.querySelector('#pwaYes').addEventListener('click', async () => { el.remove(); e.prompt(); try { await e.userChoice; } catch (_) {} localStorage.setItem('mbv_pwa_dismissed', '1'); });
    el.querySelector('#pwaNo').addEventListener('click', () => { el.remove(); localStorage.setItem('mbv_pwa_dismissed', '1'); });
  });
}
